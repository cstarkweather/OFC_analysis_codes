...
%run this entire code first if you want to generate the decoding electrodes
%and posterior probabilities from scratch,
%then run Fig_5
%this code presents Lasso regression for decoding, which produces similar results to
%stepwise AIC
...
for j = 1:num_subjects
    ...
    for i = 1:num_epochs
        ...
        predictorID = []; predictors = [];

        for e = 1:num_electrodes
            if subject(n).electrode(e).olf_coor > -1
                signal = sum(subject(n).electrode(e).trigger(2).high_gamma_mat(filter, sliding_epochs(i, 1):sliding_epochs(i, 2)), 2) / (length_epoch + 1);
                predictors = [predictors signal];
                predictorID = [predictorID e];
            end
        end
        ...
        [predictors_standardized, mu, sigma] = zscore(predictors);

        % Perform Lasso regression with cross-validation to select the best lambda
        [B, FitInfo] = lassoglm(predictors_standardized, response_variable, 'binomial', 'CV', 10);

        % Get the indices of selected predictors (non-zero coefficients)
        selected_predictor_indices = B(:, FitInfo.IndexMinDeviance) ~= 0;
